...
const wrapper = $('.assessment-tool-pdf');
const selectedCategories = [];
const selectedCategoriesObject = [];
let results = {};
const jsonURL = '/api/CategoriesData';
...
function analysisInteraction() {
    ...
    selectedCategoriesObject.forEach((category, categoryIndex) => {
        if (category.Selected) {
            ...
            // make tabs
            var assecategoryTab = category.Category == "CAPABILITY RESOURCE" ? "CAPABILITY & RESOURCE" : category.Category;

            tabsTriggersEl += `<div data-category="${category.Category}" class="tab-trigger ${tabClass}">
				${assecategoryTab.toLowerCase()}
			</div>`;
            ...
            var assecategory = category.Category == "CAPABILITY RESOURCE" ? "CAPABILITY & RESOURCE" : category.Category;

            tabsContentEl += `
				<div class="tab-content" data-category="${category.Category}">
					<div class="title">${assecategory}</div>
					${graph}
        ...
        let categoryName = category.Category.replace(/\s/g, '-');
    ...
    var isSitecoreFormVisible = $('#sitecoreform').val();

    if (isSitecoreFormVisible == "yes") {
        $("input[data-sc-field-name='pdf-html']").val(htmlEncode($(".pdf-generator-html").html()));        $("input[data-sc-field-name='pdf-json']").val(htmlEncode(JSON.stringify(results)));
    }
    else {
        $("input[name='pdfhtml']").val(htmlEncode($(".pdf-generator-html").html()));
        $("input[name='pdfjson']").val(htmlEncode(JSON.stringify(results)));
    }
}
...
function analysisTabsInteraction() {
    ...
    if (tab) {
        ...
        initialCategory = initialCategory.replace('&', '\\&');
        ...
        function animateSVG(nextCategory, lastCategory) {
            let next = nextCategory.replace(/\s/g, '-');
            let last = lastCategory.replace(/\s/g, '-');
            next = next.replace('&', '\\&');
            last = last.replace('&', '\\&');
    ...
    if (form) {
        ufValidation('.assessment-form', {
            ...
            onSuccess: () => {
                ...
                if (isSitecoreFormVisible == "yes") {
                    results.Email = form.find('.asesment-emailid').val();
                    results.FirstName = form.find('.asesment-first-name').val();
                    results.LastName = form.find('.asesment-last-name').val();
