...
function ufValidation(formSelector, options) {
    ...
    function validateInput(input) {
        ...
        if (input.attr('data-val-required') || input.attr('required')) {
            // remove error message when validating
            input.parent().find('.error-message').remove();

            // if type email
            if (input.attr('type') == 'email') {
                var acceptdomains = $("#acceptdomains").val();
                var errormesg = $("#errormessage").val();
                var acceptdomainsArray = acceptdomains.split(',');
                var currentdomain = input.val().split("@")[1];
                ...
                else if (acceptdomainsArray.includes("@" + currentdomain)) {
                    addErrorInvalid(input, errormesg);
                }
                else {
